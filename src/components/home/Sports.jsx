...
export default function Sports() {
  return (
    <section className="section" style={{background:'#0E1428'}}>
        ...
        <div className="grid-4">
          {sports.map((s, i) => (
            <div key={i} className="sport-card" style={{
              ...
              <div style={{width:'64px',height:'64px',borderRadius:'18px',background:`${s.color}15`,border:`1px solid ${s.color}25`,display:'flex',alignItems:'center',justifyContent:'center'}}>
      ...
      <style>{`.sport-card{transition:border-color 0.25s,transform 0.25s,box-shadow 0.25s;}.sport-card:hover{transform:translateY(-4px);border-color:rgba(0,212,255,0.4) !important;box-shadow:0 12px 32px rgba(0,212,255,0.12);}`}</style>
    </section>
  );
}

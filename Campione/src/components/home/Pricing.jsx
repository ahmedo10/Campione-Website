        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'24px',alignItems:'start',marginBottom:'64px'}}>
          {plans.map((p, i) => (
            <div key={i} style={{
              ...
              {p.badge && (
                <span style={{position:'absolute',top:'-14px',left:'50%',transform:'translateX(-50%)',padding:'5px 16px',borderRadius:'100px',background:'linear-gradient(135deg,#F59E0B,#EF4444)',color:'#080C1A',fontSize:'12px',fontWeight:700,whiteSpace:'nowrap'}}>
                  {p.badge}
                </span>
              )}
